...
- **Default model** `NFR-MODEL-DEFAULT` = newest Claude Opus (4.8 as of 2026-06-15, `anthropic.claude-opus-4-8`; cross-region profile `us.anthropic.claude-opus-4-8` preferred for availability) — exact id resolved at runtime from config; pinned default confirmed at implementation time.
