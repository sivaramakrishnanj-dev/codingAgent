...
| **AC-2.2** | U | The agent shall express the task breakdown as an ordered list of discrete tasks, each with a stable identifier of the form `T-<n>` or `T-<n>.<m>` (the hyphen is mandatory). For a greenfield project the breakdown is authored by the model under the greenfield playbook prompt, which **emits** this id form so the project's own breakdown conforms to the strict traceability gate (ADR-0012, DCR-2 multi-turn dialogue, DCR-5 prompt-emits-vocabulary). The gate's `TaskTraceability` **recognition is unchanged in STRICTNESS** — which lines count as tasks (`T-<n>`/`T-<n>.<m>`, hyphen mandatory) and which refs count as traces (`US-`/`AC-`/`NFR-`/`RD-`/`INV-`) is not relaxed; a miscounting-only hardening (deduplicating a repeated id, skipping an arrow/sequencing-diagram line, expanding a range heading so each task in the range is individually recognized, and recognizing a bold-wrapped id in a table row) is permitted because it changes recognition **COVERAGE, not strictness** (ADR-0012, DCR-6). The same-line-ref strictness is **NOT** loosened into a block scan (the rejected DCR-5 Option b stands rejected): single-line task rows are guaranteed by the prompt (DCR-6 T-3.7), and the hardening only stops the parser miscounting the shapes a real breakdown contains. | US-2, ADR-0012 |
