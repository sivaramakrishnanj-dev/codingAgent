...
| C3  | **Workflow drivers**       | Greenfield (discuss→design→tasks→implement) and brownfield (understand→change) playbooks over C2. Each greenfield pre-approval phase is a **multi-turn conversation** (the phase transcript carries across turns *within* the phase, so the model sees its own prior turns); the developer is offered the approval prompt each round and **approve = finalize**. On phase **approval**, greenfield **authors the deliverable deterministically in code** — it captures the model's latest substantive deliverable prose from the phase conversation and writes the target-repo artifact via `GreenfieldArtifactStore.write()` (ADR-0012); persistence does not depend on a model-emitted tool call. A non-approve answer keeps the phase conversation going (another refining turn), not persist-and-stop. Later phases inject approved earlier-phase artifact content into their phase conversation (cross-phase continuity, DCR-1). **Phase-state is resumable (DCR-3, AC-7.6):** on greenfield session start the driver **reconstructs its phase-state from the target repo's on-disk artifacts** — a phase whose artifact bears the AC-1.5 approval stamp is approved; the current phase is the first unstamped/absent one — and **resumes there** rather than restarting at requirements; a transient mid-phase failure left the failed phase's artifact unstamped, so it is retryable in place. The AC-1.5 stamp is both the resume marker and the D13 clobber-protection signal (`GreenfieldArtifactStore.write()` refuses to truncate a stamped artifact). The in-phase multi-turn transcript is not preserved across an interruption (re-enter at the phase boundary — accepted tradeoff, ADR-0012). | Mode is fixed for a session; greenfield gates on approval before writing source (source-write tools withheld across every pre-approval turn); a non-approve turn never finalizes; **phase-deliverable persistence is driver-guaranteed at approval, not model-tool-dependent** (ADR-0012); **greenfield phase-state is re-derivable from the stamped on-disk artifacts and resumable, keyed to the target project (AC-7.3 repo key)** | US-1/2/3/4/5, US-7 (AC-7.6)            | ADR-0012, ADR-0005 |
| C4  | **Model Client**           | One adapter over Bedrock Converse: build request, stream response, surface `stopReason`/`usage`; honor credential chain. **Call timeouts (DCR-4):** constructs the `BedrockRuntimeClient` with `apiCallTimeout` = the response budget and an Apache `httpClientBuilder` whose `socketTimeout` = response budget / `connectionTimeout` = connect budget — both from `ResolvedConfig` (`bedrockCallResponseTimeoutSeconds` default 300, `bedrockCallConnectTimeoutSeconds` default 10), wiring `NFR-BEDROCK-CALL-TIMEOUT` (ADR-0001). | Provider-agnostic surface; no business logic; read/invoke only; every Converse call bounded by the NFR-BEDROCK-CALL-TIMEOUT budget (response timeout counts toward the retry budget) | US-8, NFR-MODEL-*, NFR-AWS-CREDENTIALS, NFR-BEDROCK-CALL-TIMEOUT | ADR-0002, ADR-0011, ADR-0001 |
...
| C15 | **Session/Lineage store**  | Persist sessions keyed by repo; record `derived-from`/`spawned-by` edges. **Repo-keying (AC-7.3, brought forward by DCR-3):** sessions are keyed by the real repo key — git remote URL when present, else normalized absolute path (ADR-0005) — **replacing the fixed `Main.ONE_SHOT_LINEAGE` M0 placeholder** that made distinct runs over different target projects collide. This real keying is the scope for a resumable greenfield session (C3, AC-7.6): a fresh greenfield run resolves the target project's repo key, then re-derives phase-state from that project's `design/` artifacts. | Originals never deleted on compaction; sessions scoped to the real repo key (AC-7.3), not a fixed placeholder | US-7/15, RD-8, AC-7.3                   | ADR-0005           |
...
**Output-token budget on the Converse request (C4 → Model Client; amended DCR-2, 2026-06-23).** The Converse request the Model Client (C4) builds sets `inferenceConfig.maxTokens` to bound the model's output. Without it, the Bedrock backend applies its default 4096 output-token cap, which can truncate a large generation at `stopReason: MAX_TOKENS` (observed live while a greenfield phase produced a full design/tasks deliverable). The greenfield phases (C3) set an explicit budget of **16384 tokens (16K), configurable** — ample headroom for a full requirements/design/tasks markdown deliverable, well within the Claude Opus 4.x family's output ceiling (Opus 4.8/4.7/4.6 = 128K, Opus 4.5 = 64K — verified 2026-06-23). This is the **model's output cap**, distinct from `NFR-OUTPUT-MAX-INLINE` (16 KB), which governs tool/command *output disposal* into context (C6, ADR-0006) — a different axis. See ADR-0012 ("Greenfield-phase output-token budget"). `04-apis.md` § 3 already documents `inferenceConfig (maxTokens/…)` as part of the request shape; DCR-2 sets an actual value on the greenfield path.

**Call timeouts on the Converse client (C4 → Model Client; amended DCR-4, 2026-06-24).** The Model Client (C4) bounds every Converse call with the `NFR-BEDROCK-CALL-TIMEOUT` budget instead of relying on the AWS SDK's own defaults: `apiCallTimeout` = the **overall response timeout** (default 300 s) and an Apache `httpClientBuilder` (`software.amazon.awssdk:apache-client`) with `socketTimeout` = the response timeout (covers streaming responses incl. extended thinking) and `connectionTimeout` = the **connect timeout** (default 10 s). Both values come from `ResolvedConfig` via the config keys `bedrockCallResponseTimeoutSeconds` / `bedrockCallConnectTimeoutSeconds` (AC-8.10 / AC-8.11, `06-formal/resolved-config.schema.json`). A timed-out call is a retryable failure that counts toward `NFR-BEDROCK-MAX-RETRIES` (§ 3.2) — exhaustion → exit 4. This is the model-side mirror of the already-wired command-side `NFR-CMD-TIMEOUT` (`commandTimeoutSeconds`, C10); the Bedrock call timeout was specified at design time but never wired, which DCR-4 closes. See ADR-0001 ("Call timeouts"); implemented by M4 task T-4.6.
