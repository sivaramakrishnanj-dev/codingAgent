...
- **⚠ THIRD CREDENTIAL PATH — API keys / bearer token (affects NFR-AWS-CREDENTIALS, RD-11):** Bedrock now supports bearer-token auth via `AWS_BEARER_TOKEN_BEDROCK` env var (or `Authorization: Bearer` header), in addition to SigV4 (profile/default-chain). Two kinds: **short-term** (≤12h, inherits IAM principal perms, recommended for prod) and **long-term** (IAM-user-backed, exploration only). **Java token generator exists:** `software.amazon.bedrock:aws-bedrock-token-generator:1.1.0` (`BedrockTokenGenerator.builder().build().getToken()`), with cache/auto-refresh. → DECISION for user: keep profile-first+default-chain primary, and ALSO honor `AWS_BEARER_TOKEN_BEDROCK` when set? Lean yes (SDK-native, cheap, prod-friendly). → updates the credential ADR + possibly RD-11/NFR-AWS-CREDENTIALS.
