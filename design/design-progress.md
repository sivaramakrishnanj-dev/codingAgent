...
**Phase 5 amendments to date (greenfield M3, T-3.2):** **DCR-1** made greenfield phase-deliverable persistence driver-authored in code (`GreenfieldArtifactStore.write()`) rather than dependent on a model `write_artifact` tool call. **DCR-2** made each greenfield pre-approval phase a **multi-turn conversation** with **approve = finalize** (the in-phase transcript carries within a phase; approval captures the converged deliverable, persists it, stamps AC-1.5, advances; a non-approve answer is another refining turn, not persist-and-stop), and folded in the D1 output-token-cap fix (set `inferenceConfig.maxTokens = 16384`, configurable, on the greenfield Converse request so a large deliverable is not truncated at the default 4096 cap). **DCR-3** (this amendment, T-3.2-RD-D12-D13) made greenfield phase-state **resumable by re-deriving it from the on-disk approval-stamped artifacts**: a phase whose target-repo artifact bears the AC-1.5 stamp is approved; a fresh `--mode greenfield` resumes at the first unstamped/absent phase rather than restarting at requirements; a transient mid-phase failure left the failed phase unstamped, so it is retryable in place. It brought the real **AC-7.3 repo-keying forward** (git remote URL else normalized abs path, ADR-0005) to scope a resumable greenfield session, replacing the fixed `Main.ONE_SHOT_LINEAGE` M0 placeholder (the run-collision root cause), and documented that the **AC-1.5 stamp doubles as the D13 clobber-protection signal** (`GreenfieldArtifactStore.write()` refuses to truncate a stamped artifact — `ApprovedArtifactProtectedException`, code already shipped). Accepted tradeoff (user-confirmed): the in-phase multi-turn transcript is not preserved across an interruption — resume at the phase boundary and re-converse. New: AC-7.6, T-3.4, CT-GF-1/CT-GF-2 (contract-tests § 7). DCR-1/DCR-2 guarantees all retained. See ADR-0012 + `reviews/2026-06-23-amendment-greenfield-resume-r1.md`.
...
- Greenfield mid-flow resume contract (DCR-3): greenfield phase-state is **re-derived from the on-disk approval-stamped artifacts**, not separately persisted — an AC-1.5-stamped phase artifact = approved, resume at the first unstamped/absent phase; transient mid-phase failure is retryable in place (failed phase unstamped). Pinned by **AC-7.6** (`00-requirements.md` US-7), ADR-0012 ("Greenfield mid-flow resume" bullet), `02-architecture.md` § 1.2 (C3 reconstruction; C15 repo-keying), T-3.4 (`07-tasks.md` M3), CT-GF-1/CT-GF-2 (`06-formal/contract-tests.md` § 7). The **AC-1.5 stamp is one durable on-disk fact serving both the resume marker AND the D13 clobber-protection guard** (`ApprovedArtifactProtectedException`, AC-1.2/AC-1.5). Resumable greenfield sessions keyed by the **real AC-7.3 repo key** (git remote else normalized abs path, ADR-0005), replacing the `Main.ONE_SHOT_LINEAGE` M0 placeholder. Accepted tradeoff: in-phase transcript not preserved across an interruption — resume at the phase boundary, re-converse.
